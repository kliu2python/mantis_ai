#!/usr/bin/env python3
"""
Fast Mantis Scanner that estimates issue URLs instead of collecting them from pages
Gets project information from each issue's Category field
Jumps directly to issue processing with multiprocessing
"""
...
MAX_WORKERS = 20  # Number of concurrent workers for issue processing
...
# Issue ID range configuration
MIN_ISSUE_ID = 1  # Minimum issue ID to check
MAX_ISSUE_ID = 1500000  # Maximum issue ID to check
ISSUES_TO_PROCESS = 100000  # Number of issues to process (sample for testing)
...
def _extract_issue_details_with_context(context_args: Dict, issue_url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            # Check if this is a valid issue page (not a 404 or error page)
            page_title = page.title()
            if "error" in page_title.lower() or "not found" in page_title.lower():
                print(f"    Skipping invalid issue page: {issue_url}")
                page.close()
                browser.close()
                return {}
...
# ========= 🚀 Multiprocessing Issue Processing Functions ========= #
...
def generate_sample_issue_urls(sample_size: int = ISSUES_TO_PROCESS) -> List[Dict[str, str]]:
    """
    Generate a sample of issue URLs to process
    Instead of collecting from pages, we randomly sample from the known issue ID range
    """
    print(f"Generating sample of {sample_size} issue URLs from ID range {MIN_ISSUE_ID}-{MAX_ISSUE_ID}")

    issue_urls = []
    sampled_ids = set()

    # Generate random issue IDs within the range
    while len(issue_urls) < sample_size:
        issue_id = random.randint(MIN_ISSUE_ID, MAX_ISSUE_ID)

        # Avoid duplicates
        if issue_id not in sampled_ids:
            sampled_ids.add(issue_id)
            issue_urls.append({
                "issue_id": str(issue_id),
                "url": f"{MANTIS_BASE_URL}bug_view_page.php?bug_id={issue_id}"
            })

        # Progress indicator
        if len(issue_urls) % 10000 == 0:
            print(f"Generated {len(issue_urls)}/{sample_size} issue URLs")

    print(f"Generated {len(issue_urls)} issue URLs for processing")
    return issue_urls

def scan_all_mantis_issues():
    """
    Main function to scan Mantis issues by estimating URLs instead of collecting from pages
    """
    print("Starting Fast Mantis scanner with estimated issue URLs...")
    print("Project information will be extracted from each issue's Category field")
    print(f"Using {MAX_WORKERS} concurrent workers for issue processing")
    print(f"Processing a sample of {ISSUES_TO_PROCESS} issues (adjust ISSUES_TO_PROCESS to scan more)")
    ...
    try:
        # Generate sample issue URLs instead of collecting from pages
        print("\nGenerating sample issue URLs...")
        all_issue_urls = generate_sample_issue_urls(ISSUES_TO_PROCESS)
        ...
        print(f"\n=== SCAN COMPLETE ===")
        print(f"Processed {len(all_issue_urls)} issues (sample)")
