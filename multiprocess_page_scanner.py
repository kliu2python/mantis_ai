#!/usr/bin/env python3
"""
Mantis Scanner that processes ALL issues from All Projects view with multiprocessing for page collection
...
MAX_PAGE_WORKERS = 5  # Number of concurrent workers for page collection
MAX_ISSUE_WORKERS = 10  # Number of concurrent workers for issue processing
...
def get_issue_urls_from_page_worker(page_number: int, context_args: Dict) -> List[Dict[str, str]]:
    ...
    try:
        print(f"  Processing page {page_number} in worker")
...
def collect_all_issue_urls(total_pages: int, context_args: Dict) -> List[Dict[str, str]]:
    """
    Collect all issue URLs from all pages using multiprocessing
    """
    print(f"Collecting issue URLs from all {total_pages} pages using {MAX_PAGE_WORKERS} workers...")

    all_issue_urls = []

    # Create page numbers to process
    page_numbers = list(range(1, total_pages + 1))

    # Use ThreadPoolExecutor for concurrent page processing
    # (Playwright doesn't work well with multiprocessing, so we use threading)
...
class IssueProcessor:
    def __init__(self, max_workers=MAX_ISSUE_WORKERS):
...
def scan_all_mantis_issues():
    ...
    """
    print("Starting Multiprocess Mantis scanner with All Projects approach...")
    print("Project information will be extracted from each issue's Category field")
    print(f"Using {MAX_PAGE_WORKERS} concurrent workers for page collection")
    print(f"Using {MAX_ISSUE_WORKERS} concurrent workers for issue processing")
    ...
    try:
        ...
        print(f"\nCollecting issue URLs from all {total_pages} pages using multiprocessing...")
        all_issue_urls = collect_all_issue_urls(total_pages, context_args)

        print(f"\nCollected {len(all_issue_urls)} issue URLs from {total_pages} pages")

        # Create issue processor
        processor = IssueProcessor(max_workers=MAX_ISSUE_WORKERS)
        ...
        print(f"\n=== SCAN COMPLETE ===")
        print(f"Processed {len(all_issue_urls)} issues from {total_pages} pages")
