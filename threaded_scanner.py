#!/usr/bin/env python3
"""
Threaded Mantis Scanner that processes ALL issues from All Projects view
Gets project information from each issue's Category field
Uses threading for faster processing of large datasets
"""
...
BATCH_SIZE = 20  # Number of issues to process in each batch
DB_COMMIT_BATCH_SIZE = 50  # Number of records to commit to DB at once
...
# ========= 🚀 Threading Functions ========= #
def worker_thread(task_queue, result_queue, worker_id):
    """
    ...
    while True:
        try:
            ...
            issue_info = task_queue.get(timeout=5)
...
def process_issues_threaded(issue_urls: List[Dict[str, str]]) -> List[Dict[str, Any]]:
    """
    Process issues using multiple worker threads
    """
    print(f"Starting threaded processing with {MAX_WORKERS} workers for {len(issue_urls)} issues")
    ...
    for i in range(MAX_WORKERS):
        t = threading.Thread(target=worker_thread, args=(task_queue, result_queue, i))
        t.start()
        threads.append(t)
    ...
    try:
        ...
        while processed_count < len(issue_urls):
            try:
                ...
                if not any(t.is_alive() for t in threads):
                    # All threads finished but we haven't gotten all results
                    # This might indicate an issue
                    print("Warning: All threads finished but not all results collected")
                    break
                continue
        ...
        for t in threads:
            t.join(timeout=5)
            if t.is_alive():
                print(f"Warning: Worker thread {t.ident} did not terminate gracefully")
...
def scan_all_mantis_issues():
    """
    Main function to scan ALL Mantis issues from All Projects view using threading
    """
    print("Starting Mantis scanner with All Projects approach (threading)...")
    ...
    try:
        with sync_playwright() as p:
            ...
            # Process issues using threading
            print(f"\nExtracting detailed information from {len(all_issue_urls)} issues using threading...")
            all_issues_data = process_issues_threaded(all_issue_urls)
