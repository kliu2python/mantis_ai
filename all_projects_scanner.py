#!/usr/bin/env python3
"""
Mantis Scanner that processes ALL issues from All Projects view
...
def _extract_issue_details_with_context(context_args: Dict, issue_url: str) -> Dict[str, Any]:
    ...
    """
    # Create a temporary Python script to run the Playwright code
    script_content = f"""
import json
import os
from playwright.sync_api import sync_playwright

# Context args passed from parent
context_args = {repr(context_args)}
issue_url = {repr(issue_url)}
...
"""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return json_module.loads(json_output)
        else:
...
def scan_all_mantis_issues():
    """
    Main function to scan ALL Mantis issues from All Projects view
    """
    print("Starting Mantis scanner with All Projects approach...")
    ...
    try:
        with sync_playwright() as p:
            ...
            pages_to_process = max(5, total_pages)  # Process first 5 pages for testing
            ...
            # Process issues and extract detailed information
            print(f"\nExtracting detailed information from {len(all_issue_urls)} issues...")
            all_issues_data = []

            for i, issue_info in enumerate(all_issue_urls, 1):
                print(f"  Processing issue {i}/{len(all_issue_urls)}: {issue_info.get('issue_id')}")
                ...
                if detailed_issue:
                    ...
                    if len(all_issues_data) >= 10:
                        store_issues_data(all_issues_data)
                        all_issues_data = []
