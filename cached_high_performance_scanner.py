#!/usr/bin/env python3
"""
Cached High-Performance Mantis Scanner
Skips page collection if cached URLs exist, processes all issues with complete information
"""
...
ISSUE_CACHE_FILE = "collected_issue_urls.pkl"  # Cache file for issue URLs
...
# Target: 740K issues in 10 hours = 20.6 issues/sec minimum
...
# ========= 💾 Cache Management ========= #
def save_issue_urls_to_cache(issue_urls: List[Dict[str, str]]):
    """Save collected issue URLs to cache file"""
    try:
        with open(ISSUE_CACHE_FILE, 'wb') as f:
            pickle.dump(issue_urls, f)
        print(f"Saved {len(issue_urls)} issue URLs to cache file: {ISSUE_CACHE_FILE}")
    except Exception as e:
        print(f"Error saving issue URLs to cache: {e}")

def load_issue_urls_from_cache() -> List[Dict[str, str]]:
    """Load issue URLs from cache file if it exists"""
    if os.path.exists(ISSUE_CACHE_FILE):
        try:
            with open(ISSUE_CACHE_FILE, 'rb') as f:
                issue_urls = pickle.load(f)
            print(f"Loaded {len(issue_urls)} issue URLs from cache file: {ISSUE_CACHE_FILE}")
            return issue_urls
        except Exception as e:
            print(f"Error loading issue URLs from cache: {e}")

    print("No cached issue URLs found, will collect from pages")
    return []
...
def collect_all_issue_urls_parallel(total_pages: int, context_args: Dict) -> List[Dict[str, str]]:
    ...
    # Save to cache
    save_issue_urls_to_cache(all_issue_urls)
...
class HighPerformanceIssueProcessor:
    def __init__(self, max_workers=ISSUE_WORKERS):
        ...
        self.pending_results = []  # Store pending results for batch processing
            ...
            while not self.shutdown_event.is_set():
                try:
                    ...
                    if result:
                        ...
                        if batch_callback and len(self.pending_results) >= DB_COMMIT_BATCH_SIZE:
                            try:
                                batch_callback(self.pending_results)
                            except Exception as e:
    ...
    def stop_workers(self, batch_callback=None):
        """Stop all worker threads gracefully and flush remaining results"""
        ...
        # Flush any remaining results that didn't fill a complete batch
        if batch_callback and self.pending_results:
            print(f"Flushing remaining {len(self.pending_results)} results...")
            try:
                batch_callback(self.pending_results)
            except Exception as e:
                print(f"Error flushing remaining results: {e}")
            finally:
                self.pending_results.clear()
    ...
    def process_issues(self, issue_urls: List[Dict[str, str]], batch_callback=None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.stop_workers(batch_callback)
...
def scan_all_mantis_issues():
    ...
    """
    print("Starting Cached High-Performance Mantis scanner...")
    print("Project information will be extracted from each issue's Category field")
    print(f"Using {PAGE_WORKERS} concurrent workers for page collection (if needed)")
    print(f"Using {ISSUE_WORKERS} concurrent workers for issue processing")
    print(f"Target: Process 740K issues in 10 hours (20.6 issues/sec minimum)")
    ...
    try:
        # Try to load issue URLs from cache first
        all_issue_urls = load_issue_urls_from_cache()

        if not all_issue_urls:
            # No cache found, collect issue URLs from pages
            print("No cached issue URLs found, collecting from pages...")
            ...
            print("Using cached issue URLs, skipping page collection phase")

        # Process ALL collected issues (no limiting for 740K)
        target_issues = len(all_issue_urls)
        print(f"\nProcessing all {target_issues} collected issues")
        ...
        def store_batch(results):
            if results:
                ...
                try:
                    success = store_issues_data_batch(results)
                    if not success:
                        print("Warning: store_issues_data_batch returned False")
                except Exception as e:
        ...
        target_rate = 20.6  # issues per second for 740K in 10 hours
        if rate >= target_rate:
        ...
        # Time estimation for 740K issues
        estimated_time_hours = (740000 / rate) / 3600 if rate > 0 else 0
        print(f"Estimated time for 740K issues: {estimated_time_hours:.1f} hours")
